package com.example.ts.grallery.view;
...
import com.example.ts.grallery.utils.ImagePickerProvider;
    ...
    public void saveBitmapToFile(File folder, int expectWidth, int exceptHeight, boolean isSaveRectangle) {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                // 把文件插入到系统图库
                String path = finalSaveFile.getAbsolutePath();
                String fileTitle = path.substring(path.lastIndexOf("/"));
                try {
                    MediaStore.Images.Media.insertImage(getContext().getContentResolver(),
                            finalSaveFile.getAbsolutePath(), fileTitle, null);
                } catch (FileNotFoundException e) {
                    e.printStackTrace();
                }
                // 通知图库更新
                getContext().sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE,
                        ImagePickerProvider.getUriForFile(getContext(),"com.example.ts.grallery.provider",finalSaveFile)));
            }
        }.start();
    }
